...
if (-not (Test-Path $prebuildDir)) { New-Item -ItemType Directory -Path $prebuildDir }
if (-not (Test-Path $packDir)) { New-Item -ItemType Directory -Path $packDir }
...
if (-not (Test-Path "$tempDir\$BuildVersion-pre.7z")) {
    Invoke-WebRequest -Uri "https://7-zip.org/a/$($BuildVersion.Replace('7z',''))-x64.exe" -OutFile "$tempDir\$BuildVersion-pre.7z"
...
if (-not (Test-Path "$outDir\Lang")) { New-Item -ItemType Directory -Path "$outDir\Lang" }
